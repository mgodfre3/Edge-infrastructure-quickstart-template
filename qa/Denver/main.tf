 module "base" {
   ...
   startingAddress = "172.22.81.30"
   endingAddress   = "172.22.81.39"
   ...
   servers = [
     {
       name        = "Den-Node1",
       ipv4Address = "172.22.81.21"
     },
     {
       name        = "Den-Node2",
       ipv4Address = "172.22.81.22"
     }
   ]
  ...
  #  Region HCI logical network parameters
   lnet-startingAddress = "172.22.81.40"
   lnet-endingAddress   = "172.22.81.55"   #This IP range should not overlap with HCI infra IP range.
   lnet-addressPrefix   = "172.22.81.0/24"   #E.g., 192.168.1.0/24
   lnet-defaultGateway  = "172.22.81.1" # Default gateway can be same as HCI infra default gateway.
   lnet-dnsServers      = ["10.254.0.196", "10.254.0.197"]   # DNS servers can be same as HCI infra DNS servers.

#   # Region AKS Arc parameters
   aksArc-controlPlaneIp   = "172.22.81.56"      # An IP address in the logical network IP range.
