 module "base" {
  source          = "../../modules/base"
   ...
   lnet-startingAddress = "10.20.242.20"
   lnet-endingAddress   = "10.20.242.100"   //This IP range should not overlap with HCI infra IP range.
   lnet-addressPrefix   = "10.20.242.0/24"   //E.g., 192.168.1.0/24
   lnet-defaultGateway  = "10.20.212.1"  //Default gateway can be same as HCI infra default gateway.
   lnet-dnsServers      = ["10.254.0.196", "10.254.0.197"]   // DNS servers can be same as HCI infra DNS servers.

   # Region AKS Arc parameters
   aksArc-controlPlaneIp   = "10.20.242.110"       //An IP address in the logical network IP range.
